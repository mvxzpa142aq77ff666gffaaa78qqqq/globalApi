...
async function ObtenerDeCajaAMasterTodo(req, res) {
    const { id } = req.params

    try {

        const result = await MovimientoDeCajaAMaster.paginate({},{limit: 1000,sort: { createdAt: -1 }} )
        console.log(result)
...
module.exports = ObtenerDeCajaAMasterTodo