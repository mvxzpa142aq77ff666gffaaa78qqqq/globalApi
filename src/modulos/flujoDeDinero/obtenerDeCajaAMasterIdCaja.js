...
async function ObtenerDeCajaAMasterIdCaja(req, res) {
    const { id } = req.params

    try {

        const result = await MovimientoDeCajaAMaster.paginate({idCaja:id},{limit: 1000,sort: { createdAt: -1 }} )
        console.log(result)
...
module.exports = ObtenerDeCajaAMasterIdCaja