...
async function ObtenerRecargasPorIdMaster(req, res) {
    const { id } = req.params

    try {

        const result = await RecharSolde.paginate({idMaster:id},{limit: 1000,sort: { createdAt: -1 }} )
        console.log(result)
...
module.exports = ObtenerRecargasPorIdMaster