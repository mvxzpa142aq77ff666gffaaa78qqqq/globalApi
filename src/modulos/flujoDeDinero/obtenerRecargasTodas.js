...
async function ObtenerTodasLasRecargas(req, res) {
    const { id } = req.params

    try {
        const result = await RecharSolde.paginate({},{limit: 1000,sort: { createdAt: -1 }} )
        console.log(result)
...
module.exports = ObtenerTodasLasRecargas