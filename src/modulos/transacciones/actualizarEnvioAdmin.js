...
const arrayAdmin = ['Gestor', 'Cajero', 'Atencion_al_cliente', 'Atencion_al_cliente', 'super_admin']



async function ActualizarEnvioAdmin(req, res) {
    ...
    const verifyAdmin = await RegisControlSist.findById(idAdmin).populate({ path: 'roles' })
    ...
    try {
        ...
        if (resultEnvio !== null && !resultEnvio.verifyRecp) {
            ...
            if (verifyAdmin !== null && verifyAdmin.activeCount) {

                if (arrayAdmin.includes(verifyAdmin.roles[0].name)) {
                    ...
                    res.status(200).json({ "verificar": false, "mens": "Admin no existe" })
...
module.exports = ActualizarEnvioAdmin